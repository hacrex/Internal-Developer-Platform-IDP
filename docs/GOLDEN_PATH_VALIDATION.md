...
The `service-with-observability` template is intentionally small. It creates a service skeleton with catalog metadata, a health endpoint, structured startup logging, a container definition, and a first-response runbook.
...
4. Confirm the rendered repository contains `catalog-info.yaml`, `docs/runbook.md`, `Dockerfile`, and `src/server.js`.
